...
class ActorCriticAgent:
    ...
    def train(self):
        ...
        while num_iter < self.config.max_train_iterations:
            ...
            if is_gameover:
                ...
                self.num_games += 1
                self.num_wins += int(is_win)
                self.best_score = max(self.best_score, self.game_agent.score)
            ...
            if len(frames) == self.config.num_continuous_frames:
                ...
                self.log('[INFO] 训练迭代: {}, Actor Loss: {}, Critic Loss: {}, [BEST SCORE]: {}, [NUMWINS/NUMGAMES]: {}/{}'.format(num_iter, actor_loss.item(),
                                                                                           critic_loss.item(),self.best_score, self.num_wins, self.num_games))
    ...
    def test(self):
        ...
        total_score = 0
        num_games = 0
        while num_games < self.config.num_test_games:
            frame, is_win, is_gameover, reward, _ = self.game_agent.nextFrame(None)
            if is_gameover:
                self.game_agent.reset()
                num_games += 1
                total_score += self.game_agent.score

            frame = self.preprocess_image(frame)
            frames.append(frame)
            if len(frames) == self.config.num_continuous_frames:
                image = np.concatenate(frames, axis=2)
                state = torch.from_numpy(image).unsqueeze(0).to(self.device)
                with torch.no_grad():
                    action_probs = self.actor(state)
                    action = action_probs.max(1)[1].item()  # Choose the best action
                formatted_action = self.formatAction(action, 'oriactionformat')
                frames.pop(0)
